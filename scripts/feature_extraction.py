...
n_features = 10
print("PRO-TRUMP FEATURES INDICES")
anti_features = twitter.fc1.weight[0].data.cpu().numpy()
anti_features_max_indices = anti_features.argsort()[-n_features:][::-1]
print(anti_features_max_indices)

print("ANTI-TRUMP FEATURES INDICES")
pro_features = twitter.fc1.weight[1].data.cpu().numpy()
pro_features_max_indices = pro_features.argsort()[-n_features:][::-1]
print(pro_features_max_indices)

tweets = []
# load up data
print("Loading tweets...")
data = []
with open('/mnt/storage01/milliet/data/tweets_representation.csv', 'r') as csvfile:
  lines = csvfile.readlines()
  for line in lines:
      data.append(line.split('\sep'))

print("Get ngrams...")
tweets_data = list(" ".join((" ".join((" ".join((" ".join(elem[0][5:-2].split("', '"))).split('", \''))).split("', \""))).split('", "')) for elem in data)
...
print("Find representents of pros and antis features")
n_represent = 5
pro_features_to_data = {}
for feature in pro_features_max_indices:
    feature_str = str(feature)
    pro_features_to_data[feature_str] = []
    j=0
    for embed in embeds:
        if len(pro_features_to_data[feature_str])>n_represent:
            min_tuple = min(pro_features_to_data[feature_str], key = lambda t: t[1])
            min_val = min_tuple[1]
            if embed[feature]>min_val:
                pro_features_to_data[feature_str].remove(min_tuple)
                pro_features_to_data[feature_str].append((tweets_data[j],embed[feature]))
        else:
            pro_features_to_data[feature_str].append((tweets_data[j],embed[feature]))
        j+=1
        
anti_features_to_data = {}
for feature in anti_features_max_indices:
    feature_str = str(feature)
    anti_features_to_data[feature_str] = []
    j=0
    for embed in embeds:
        if len(anti_features_to_data[feature_str])>n_represent:
            min_tuple = min(anti_features_to_data[feature_str], key = lambda t: t[1])
            min_val = min_tuple[1]
            if embed[feature]>min_val:
                anti_features_to_data[feature_str].remove(min_tuple)
                anti_features_to_data[feature_str].append((tweets_data[j],embed[feature]))
        else:
            anti_features_to_data[feature_str].append((tweets_data[j],embed[feature]))
        j+=1

for key, list_ in pro_features_to_data.items():
    list_ordered = sorted(list_, key=lambda x: x[1])
    pro_features_to_data[key] = list_ordered

for key, list_ in anti_features_to_data.items():
    list_ordered = sorted(list_, key=lambda x: x[1])
    anti_features_to_data[key] = list_ordered

print("Write files")
proDict = {'pro_features_dict': pro_features_to_data}

with open('/mnt/storage01/milliet/data/' + str(n_features) + 'pro_features_to_data_' + str(n_represent) + '.txt', 'w') as file:
     file.write(json.dumps(proDict))
     
antiDict = {'anti_features_dict': anti_features_to_data}

with open('/mnt/storage01/milliet/data/' + str(n_features) + 'anti_features_to_data_' + str(n_represent) + '.txt', 'w') as file:
